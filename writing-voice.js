...
// Learns the writer's voice from in-app exemplars (starred letters + samples they add),
...
// Personal — all gitignored or data-home; never promote the DATA to valinor
// (the code promotes; the stores resolve per-machine).
function profilePath() { return dataHome.resolveStore({ env: 'WRITING_PROFILE_PATH', name: 'writing-voice.md', legacy: ['writing-voice.md'] }); }
function starsPath() { return dataHome.resolveStore({ env: 'WRITING_STARS_PATH', name: 'writing-stars.json', legacy: ['writing-stars.json'] }); }
function samplesPath() { return dataHome.resolveStore({ env: 'WRITING_SAMPLES_PATH', name: 'writing-samples.json', legacy: ['writing-samples.json'] }); }
function runsPath() { return dataHome.resolveStore({ env: 'WRITING_RUNS_PATH', name: 'writing-voice-runs.json', legacy: ['writing-voice-runs.json'] }); } // eval trajectory log
...
function getStars() {
  const raw = safeReadJson(starsPath(), {});
...
function getSamples() {
  const raw = safeReadJson(samplesPath(), []);
...
function getRuns() {
  const raw = safeReadJson(runsPath(), []);
  return Array.isArray(raw) ? raw : [];
}
...
function saveProfile(text) {
  fs.writeFileSync(profilePath(), String(text || ''));
  return getProfile();
}
...
const BUILD_SYSTEM = `You are a forensic prose analyst building a WRITING VOICE PROFILE for a specific writer. The profile is read by an AI ghostwriter before it rewrites their drafts, and by the writer, who hand-edits it. It must capture what makes THEIR writing recognizably theirs — a fingerprint, not a book report.

You are given two kinds of material:
- MINE — writing the author actually wrote. This is the TARGET VOICE. The ghostwriter must sound like THIS.
- INSPIRATION — writers they admire. QUALITIES to aspire to, never a voice to copy. Pull only DISTINCTIVE, non-obvious principles; the output must be the writer at their best, never a pastiche of these authors.

If an EXISTING PROFILE is provided, PRESERVE any hand-written rules the writer clearly added; refine, don't discard.

METHOD — be forensic and specific. For MINE, actually measure and observe:
- Rhythm: typical sentence-length pattern and how they vary it (e.g. "runs 2-3 long clauses then snaps to a 3-word line"). Name the pattern, don't just say "varied."
- Diction & register: his actual word choices, level of formality, where they go blunt vs. tender, technical vs. plain.
- Punctuation habits: how they really use dashes, parentheticals, colons, fragments, line breaks — with his tendencies, not generic advice.
- Openings & closings: how they actually start and land a piece. Quote real examples.
- Structure: paragraph length, use of headers/lists, how they build an argument or a reflection.
- Tics & signatures: recurring words, constructions, rhetorical moves, the way they address a reader.
...
## Voice fingerprint
- 6-10 specific, QUOTED observations from MINE covering rhythm, diction, punctuation, openings/closings, structure, and tics. Each must be something you could only say about this writer.

## Signature moves & phrases
- 4-8 verbatim phrases, constructions, or rhetorical moves they reuse — the things a reader would recognize as "that's him." Quote them.

## Good-writing rubric
- 3-6 DISTINCTIVE principles distilled from INSPIRATION (and his own best work). Actionable and non-obvious. No platitudes.

## Anti-AI-tells (never do)
- Keep the baseline list you are given verbatim, then add any writer-specific tells you infer (words they'd never use, moves that would ring false in their voice).
...
async function buildProfile({ apiKey } = {}) {
  ...
  const userParts = [
    block(mine, `MINE — the writer's own writing (the target voice, weight heavily):`),
    block(inspiration, `INSPIRATION — writing the author admires (aspire to the qualities, do NOT copy the voice):`),
    `BASELINE ANTI-TELLS to keep in the third section:\n${ANTI_TELLS.map((t) => `- ${t}`).join('\n')}`,
  ];
  if (existing.exists) {
    userParts.push(`EXISTING PROFILE (preserve the writer's hand-edits):\n${clip(existing.profile, 4000)}`);
  }
...
// The single biggest lever on voice fidelity is few-shot: the model must SEE the writer's actual
...
function rewriteSystem(profileText, exemplars) {
  ...
  const exemplarBlock = ex.length
    ? `\nVERBATIM EXCERPTS OF THE WRITER'S OWN WRITING — this is the texture you must match. Study the sentence rhythm, diction, punctuation, and how they land a point. Imitate the FEEL, never lift the content:\n\n${ex.map((e) => `--- ${e.label} ---\n${e.text}`).join('\n\n')}\n`
    : '';
  return `You are the writer's ghostwriter. You rewrite prose so it reads as if THEY wrote it on a good day — never like an AI, never like generic "good writing." Two references follow: a distilled voice profile, and verbatim excerpts of their actual writing. When they seem to conflict, trust the EXCERPTS — they are the ground truth; the profile is only a summary.

Do not invent facts or add ideas they didn't put there. Preserve their meaning and the structure of the input unless the instruction says otherwise. Keep it recognizably human and specific — match their real register, including where they are blunt, informal, or rough. Do not smooth them into something more polished or corporate than the excerpts show.
...
const REFINE_SYSTEM = `You maintain a WRITING VOICE PROFILE for a writer. They just tested the voice — rewrote a piece of text using the current profile, read the output, and left feedback on what was off or what they want more of. Update the profile so the NEXT rewrite reflects their feedback.

You are given: the CURRENT PROFILE, the test INPUT, the OUTPUT the ghostwriter produced, and the writer's FEEDBACK.
...
- Preserve every existing section, all of the writer's hand-written rules, and the exact section structure (the "#" title and "##" headers). Keep the quoted, specific style — never introduce generic writing-teacher advice.
...
- If the feedback names a tell to avoid, add it under Anti-AI-tells. If it's a move he likes, sharpen Voice fingerprint or Signature moves & phrases. Quote their own words from the feedback where it helps.
...
async function refineProfileFromFeedback({ text, output, feedback, profileOverride, runId, apiKey } = {}) {
  ...
  const user = [
    ...
    output ? `OUTPUT PRODUCED (what they're reacting to):\n${clip(String(output), 4000)}` : '',
    `WRITER'S FEEDBACK:\n${fb}`,
  ].filter(Boolean).join('\n\n');
  const content = await callOpenRouter([
