...
// Default: ~/Savar/memory/execute/ (override with EXECUTE_DIR). PRIVATE — never promoted
// to valinor.
const fs = require('fs');
...
const EXECUTE_DIR =
  process.env.EXECUTE_DIR || path.join(os.homedir(), 'Savar', 'memory', 'execute');
...
function backfill(t) {
  ...
  if (!('blockKind' in t)) t.blockKind = null;
  ...
  // Granular edit log: every manual adjustment (title/outcome/bucket/block)
  // with before → after, so estimate-vs-actual is learnable per card.
  if (!Array.isArray(t.edit_history)) t.edit_history = [];
...
function createTask({ title, outcome, sourceIngestId, comment, kind, why, intent, action_type, duration_bucket, sourceTitle, sourceBody, parentTaskId, blockKind, scheduled_block } = {}) {
  ...
  const bk = String(blockKind || '').trim().toLowerCase();
  const block = ['break', 'meal', 'focus'].includes(bk) ? bk : null;
  const task = {
    id,
    created,
    updated: created,
  ...
  if (scheduled_block && typeof scheduled_block === 'object') {
    const date = String(scheduled_block.date || '').trim();
    const start = String(scheduled_block.start || '').trim();
    const end = String(scheduled_block.end || '').trim();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || !/^\d{2}:\d{2}$/.test(start) || !/^\d{2}:\d{2}$/.test(end)) {
      const e = new Error('scheduled_block needs { date: YYYY-MM-DD, start: HH:MM, end: HH:MM }');
      e.status = 400;
      throw e;
    }
    task.scheduled_block = { date, start, end };
  }
  const tasks = readAll();
  tasks.push(task);
  writeAll(tasks);
...
function updateTask(id, { title, outcome, comment, scheduled_block, duration_bucket } = {}) {
  return mutate(id, (t) => {
    ...
    const logEdit = (field, before, after) => {
      ...
      const hist = Array.isArray(t.edit_history) ? t.edit_history : [];
      t.edit_history = [...hist, { at, field, before: before ?? null, after: after ?? null }].slice(-100);
    };
    if (typeof title === 'string') {
      const v = title.trim() || t.title;
      logEdit('title', t.title, v);
      t.title = v;
    }
    if (typeof outcome === 'string') {
      const v = outcome.trim();
      logEdit('outcome', t.outcome, v);
      t.outcome = v;
    }
    if (duration_bucket !== undefined) {
      const db = String(duration_bucket || '').trim();
      if (db && !DURATION_BUCKETS.includes(db)) {
        const e = new Error('duration_bucket must be one of ' + DURATION_BUCKETS.join(' | '));
        e.status = 400;
        throw e;
      }
      const v = db || null;
    ...
    if (scheduled_block !== undefined) {
      if (scheduled_block === null) {
        logEdit('scheduled_block', t.scheduled_block, null);
        t.scheduled_block = null;
      } else if (scheduled_block && typeof scheduled_block === 'object') {
        const date = String(scheduled_block.date || '').trim();
        const start = String(scheduled_block.start || '').trim();
        const end = String(scheduled_block.end || '').trim();
        if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || !/^\d{2}:\d{2}$/.test(start) || !/^\d{2}:\d{2}$/.test(end)) {
        ...
        logEdit('scheduled_block', t.scheduled_block, { date, start, end });
        t.scheduled_block = { date, start, end };
      }
    }
    return t;
  });
...
function unstepTask(id) {
  return mutate(id, (t) => {
    ...
    if (t.status === 'review') {
      // Back to Active keeps the proof (it stays attached) — only the Q&A resets.
      t.status = 'active';
      t.startedAt = new Date().toISOString();
      t.review = null;
      return t;
...
// Attach proof: from Active, just stores the file — status and timer are
// untouched, so attaching never advances the card on its own. The explicit
// "send to review" step (sendToReview) is what stops the clock and moves it.
// From Review, replaces the file and keeps the existing question.
async function attachProof({ id, buf, mime, filename }, apiKey) {
  ...
  return mutate(safe, (t) => {
    ...
    if (t.status === 'review' && !(t.review && t.review.question)) {
      t.review = {
        question: 'What did completing this unlock, and what is the next move?',
        answer: null,
        answeredAt: null,
      };
    }
...
// Send to review: the MANUAL completion step. Requires proof (it unlocks the
// option), stops the timer, moves Active -> Review, and asks the reflection
// question. Never happens implicitly from an upload.
async function sendToReview(id, apiKey) {
  ensureDir();
  const safe = path.basename(String(id || ''));
  const task = getTask(safe);
  if (!task) { const e = new Error('task not found'); e.status = 404; throw e; }
  if (task.status !== 'active') {
    const e = new Error('only Active tasks go to review');
    e.status = 400;
    throw e;
  }
  if (!findProof(safe, task)) {
    const e = new Error('attach proof first — it unlocks Review');
    e.status = 400;
    throw e;
  }
  let question = 'What did completing this unlock, and what is the next move?';
  try {
    question = await reviewQuestion(task, apiKey);
  } catch (e) {
