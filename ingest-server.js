...
// Default: ~/Savar/memory/ingest/ (override with INGEST_DIR). PRIVATE — never promoted
// to valinor.
const fs = require('fs');
...
const INGEST_DIR =
  process.env.INGEST_DIR || path.join(os.homedir(), 'Savar', 'memory', 'ingest');
...
// ---- markdown <-> object -------------------------------------------------
function toMarkdown({ id, created, source, title, refined, plan, nextAction, raw, image, tags, kind, threadId, threadTitle, origin, messageRef }) {
  const steps = Array.isArray(plan) ? plan : [];
  const baseTags = Array.isArray(tags) && tags.length ? tags : ['ingest'];
  const k = kind ? String(kind) : null;
  const tagList = (k && k !== 'note' && !baseTags.includes(k)) ? [...baseTags, k] : baseTags;
  const fm = [
    '---',
    `id: ${id}`,
    `created: ${created}`,
    ...
    threadId ? `thread: ${String(threadId)}` : null,
    threadTitle ? `threadTitle: ${JSON.stringify(String(threadTitle))}` : null,
    origin ? `origin: ${String(origin)}` : null,
    messageRef ? `msg: ${JSON.stringify(String(messageRef))}` : null,
    `tags: [${tagList.join(', ')}]`,
    '---',
    '',
  ...
  const items = fs
    ...
    .map((f) => {
      ...
      return {
        ...
        kind: meta.kind || 'note',
        threadId: meta.thread || null,
        threadTitle: meta.threadTitle || null,
        origin: meta.origin || null,
        messageRef: meta.msg || null,
        preview,
        image: image || null,
...
// Memory-object kinds for conversation-sourced captures.
const MEMORY_KINDS = new Set(['idea', 'recommendation', 'decision', 'thought', 'note']);

async function capture({ text, history, source, kind, threadId, threadTitle, messageRef, origin, title }, apiKey) {
  ...
  const k = MEMORY_KINDS.has(String(kind || '').toLowerCase()) ? String(kind).toLowerCase() : (src === 'conversation' ? 'thought' : 'note');
  const finalTitle = (title != null && String(title).trim()) || r.title;
  const saved = writeCapture({
    created,
    source: src,
    kind: k,
    threadId: threadId ? String(threadId).slice(0, 160) : null,
    threadTitle: threadTitle ? String(threadTitle).slice(0, 160) : null,
    messageRef: messageRef ? String(messageRef).slice(0, 160) : null,
    origin: origin === 'agent' ? 'agent' : 'manual',
    title: finalTitle,
    refined: r.refined,
    plan: [],
    nextAction: '',
  ...
  return { ...saved, ...r, title: finalTitle, kind: k, raw };
}
