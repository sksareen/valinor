// Gather the last hour of hub work and ask Valinor to write Savar a letter.
'use strict';
...
const LETTER_SYSTEM = `You are Valinor, writing a letter to Savar Sareen after a sitting of shared work.

He is Savar — never "the user", "the human", or "that person". Open with Dear Savar.
...
// labeled run with the prompt before/after snapshots, so Savar can review or
// revert any iteration.
const PROMPT_RUNS_FILE = path.join(__dirname, 'letter-session-prompt-runs.json');

function getPromptRuns() {
  const raw = safeReadJsonRuns(PROMPT_RUNS_FILE, []);
  return Array.isArray(raw) ? raw : [];
}
function writePromptRuns(runs) {
  fs.writeFileSync(PROMPT_RUNS_FILE, JSON.stringify(runs, null, 2));
  return runs;
}
...
async function refineSessionPrompt({ prompt, testTitle, testBody, feedback, runId }, apiKey) {
  ...
  const user = [
    ...
    testBody ? `TEST LETTER PRODUCED — this is what Savar is reacting to (data):\n<TEST_LETTER>\n${clip(String(testBody), 6000)}\n</TEST_LETTER>` : '',
    `SAVAR'S FEEDBACK (the lesson to fold in):\n<FEEDBACK>\n${fb}\n</FEEDBACK>`,
  ...
  if (/^# .+\n\nDear Savar,/m.test(revised) && !/^# .+\n\nDear Savar,/m.test(current)) {
    throw Object.assign(new Error('The refiner wrote a letter instead of a revised prompt — nothing saved, try again.'), { status: 502 });
  }
