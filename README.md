...
flowchart LR
  subgraph overlay ["electron overlay (optional) — transparent, click-through, always-on-top"]
    ...
    subgraph views ["views — iframes"]
      ...
      MORE["writing · rehearse · agent · convos · …"]
      HUD["breathe.html · BREATHE · SYSTEM"]
...
| `writing.html` | Writing as paper cards (content stays local; `letters/` gitignored) |
| `rehearse.html` | Voice-following teleprompter + self-view |
| `learn.html` | **LEARN** — voice spaced-repetition; study prose → quiz, cross-session strength, custom decks |
| `breaker.html` | GAME — brick breaker (hand paddle) |
| `breathe.html` | BREATHE + SYSTEM |
...
### 2026-09-12
- **WRITING — voice profile system overhaul**: the ghostwriter now sounds like Savar
  instead of generic "good writing."
  - **Few-shot rewrites**: the rewrite engine now injects verbatim excerpts of Savar's own
  ...
  - **Forensic profile builder**: the build prompt forces quoted, specific observations
    ...
    adds a "Signature moves & phrases" section and Savar-specific anti-tells.
...
- **INGEST → PLAN → EXECUTE → REVIEW loop**: capture now stays a clean *note* — the
  ...
  **Enhance** button in the ingest detail view generates one tangible outcome on demand
  ...
  private JSON store under `~/Savar/memory/execute/`): a Kanban board
...
- **LOOPS replaces TODO**: tab 1 embeds Loopkeeper via a generic `PROXIED_APPS`
  reverse proxy. Floating to-dos / `todos.json` removed; `breathe.html` keeps
  BREATHE + SYSTEM only.
