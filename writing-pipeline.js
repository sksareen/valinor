...
// voice profile from writing-voice.js so drafts land in Savar's voice.
...
const STORE_FILE = path.join(ROOT, 'writing-projects.json');
...
function draftSystem() {
  ...
  return `You ghostwrite a full article as Savar — never like an AI. Follow the picked thesis and structure. Use the research for facts and cite sources inline as [title](url) links where load-bearing. Do not invent statistics, quotes, or studies.
...
async function runDraft(id, apiKey) {
  ...
  const md = await callOpenRouter([
    ...
    { role: 'user', content: `IDEA:\n${clip(p.braindump, 3000)}\n\nRESEARCH:\n${clip(researchBriefText(p), 4000)}\n\nTHESIS OPTIONS:\n${clip(JSON.stringify(p.options || firstOpt || []), 3000)}\n\nSAVAR'S DIRECTION (follow this — it overrides the options):\n${clip(direction, 2000)}\n\nWrite the article now.` },
