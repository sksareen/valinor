// ingest-drawer.js — the GLOBAL ingest tab: a left-edge slide-out drawer on every
// main tab (EXECUTE, WRITING, …). Hover the far-left edge to peek, click the INGEST
// handle or press ⌘I to pin. Filter by source chips + text search, then click-to-send
// a capture into Execute (new backlog task with snapshotted context), Writing (new
// inbox idea), or the Live chat (draft text, ready to send). Click a title for a
// full-height preview slide-out (no new tab). Zero deps; styles derive from each
// page's CSS vars with fallbacks.
// Include once per page: <script src="ingest-drawer.js"></script> + IngestDrawer.attach().
(function () {
  ...
  var CSS = [
    ...
    '.ingest-head{display:flex;align-items:center;justify-content:space-between;padding:16px 18px 12px;',
    '  border-bottom:1px solid var(--line,#1e293b);}',
    '.ingest-title{font:700 13px Lato,system-ui,sans-serif;letter-spacing:.14em;color:var(--accent,#7dd3fc);}',
  ...
  function attach() {
    ...
    handle.textContent = 'INGEST';
    handle.title = 'Ingest (⌘I) — or hover the left edge';
    var panel = document.createElement('div');
    panel.className = 'ingest-panel';
    panel.setAttribute('aria-hidden', 'true');
    panel.innerHTML =
      '<div class="ingest-head"><div class="ingest-title">INGEST</div>' +
      '<button type="button" class="ingest-close" title="Close">✕</button></div>' +
    ...
    addEventListener('keydown', function (e) {
      if ((e.metaKey || e.ctrlKey) && String(e.key || '').toLowerCase() === 'i') {
    ...
    function render() {
      ...
      rows.slice(0, 80).forEach(function (it) {
        ...
        toAgent.textContent = 'To Agent →';
        toAgent.title = 'Drop this capture into the Live chat, ready to send';
        toAgent.onclick = function () { sendToAgent(it, toAgent, errEl); };
        if (sentMap[String(it.id) + '>agent']) preSent(toAgent, 'in Live chat');
    ...
    // To Agent — drop the full capture into the Live chat, ready to send.
    // Same handoff the Writing drawer uses: localStorage + live.html consumes it.
    // If the capture has an image, fetch + downscale it and ride it along as
    // `live-chat-image` so the Live turn carries the actual picture, not just text.
    // Already on Live? Fill #chatIn in place instead of navigating.
      ...
      try {
        ...
        if (/live\.html$/.test(location.pathname)) {
          ...
          if (typeof window.consumeChatDraft === 'function') window.consumeChatDraft();
            ...
            if (!inp) throw new Error('Live chat not found on this page.');
            inp.value = draft;
            inp.focus();
          }
          markSent(btn, 'in Live chat', it, 'agent');
          notify('agent', it.id);
        } else {
          handoff();
          markSent(btn, 'in Live chat', it, 'agent');
    ...
    function renderPreview(id, j) {
      ...
      preview.innerHTML =
        ...
        '</div>' +
        '<a class="ingest-prev-open" href="/ingest.html?id=' + encodeURIComponent(id) + '" target="_blank" rel="noopener" title="Open the full page">↗</a>' +
        '<button type="button" class="ingest-prev-x" title="Close preview">✕</button></div>' +
        ...
        '<div class="ingest-prev-actions">' +
          ...
          '<button type="button" class="ingest-btn" data-act="agent">To Agent →</button>' +
      ...
      if (sentMap[id + '>agent']) preSent(act('agent'), 'in Live chat');
  ...
  window.IngestDrawer = { attach: attach };
})();
