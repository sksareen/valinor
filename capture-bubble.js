...
//
// Hold ` (backtick) anywhere, or press-and-hold the bubble, to record. On release the
...
(function () {
  ...
  #camToggle,#cbDot{width:48px;height:48px;flex:none;border:none;border-radius:0;cursor:pointer;display:flex;align-items:center;justify-content:center;
    background:transparent;color:#7dd3fc;box-shadow:none;transition:background .12s,color .12s;padding:0;}
  #camToggle{border-right:1px solid rgba(255,255,255,.1);color:#94a3b8;}
  #camToggle:hover,#cbDot:hover{background:rgba(255,255,255,.04);}
  #camToggle.on{color:#7dd3fc;}
  #camToggle.off{color:#64748b;}
  ...
  #cbPanel{position:absolute;right:0;bottom:60px;width:340px;max-height:60vh;display:none;flex-direction:column;
    background:#0b1120;border:1px solid #1e293b;border-radius:14px;box-shadow:0 12px 40px rgba(0,0,0,.55);overflow:hidden;}
  #cbWrap.open #cbPanel{display:flex;}
  #cbHead{display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border-bottom:1px solid #1e293b;color:#94a3b8;font-size:12px;}
  #cbHead b{color:#e2e8f0;font-weight:650;letter-spacing:.02em;}
  #cbThread{overflow-y:auto;padding:10px 12px;display:flex;flex-direction:column;gap:10px;}
  .cbCard{background:#0f172a;border:1px solid #1e293b;border-radius:10px;padding:10px 12px;}
  .cbCard .t{color:#e2e8f0;font-weight:650;font-size:13px;margin-bottom:4px;}
  .cbCard .r{color:#cbd5e1;font-size:13px;line-height:1.45;}
  .cbCard .plan{margin:8px 0 0;padding-left:16px;color:#94a3b8;font-size:12.5px;line-height:1.5;}
  .cbCard .next{margin-top:8px;color:#7dd3fc;font-size:12.5px;}
  .cbCard .raw{margin-top:8px;color:#64748b;font-size:11.5px;font-style:italic;border-top:1px dashed #1e293b;padding-top:6px;}
  .cbCard.pending{color:#64748b;font-size:12.5px;font-style:italic;}
  #cbHint{padding:8px 12px;border-top:1px solid #1e293b;color:#64748b;font-size:11.5px;text-align:center;}
  #cbHint kbd{background:#1e293b;color:#cbd5e1;border-radius:4px;padding:1px 5px;font-family:ui-monospace,monospace;}
  #cbClose{cursor:pointer;color:#64748b;}#cbClose:hover{color:#cbd5e1;}
  ...
  panelEl.innerHTML = `
      ...
      <div id="cbThread"></div>
      <div id="cbHint">Hold <kbd>\`</kbd> to talk &middot; release to save</div>`;
  ...
  async function startRec() {
    ...
    discard = false;
    let stream;
    try { stream = await getMic(); } catch (e) { hint.textContent = 'mic blocked — check permission'; return; }
    ...
    open();
    hint.textContent = 'listening… release to save';
  }
  ...
  async function onStop() {
    ...
    if (discard) {
      discard = false;
      hint.innerHTML = 'Hold <kbd>`</kbd> to talk &middot; release to save';
      return;
    }
    if (blob.size < 800) {
      hint.innerHTML = 'Hold <kbd>`</kbd> to talk &middot; release to save';
    ...
    try {
      ...
      p.remove();
      renderCard(cj);
      hint.innerHTML = 'saved &middot; Hold <kbd>\`</kbd> to talk';
    } catch (e) {
      p.className = 'cbCard';
  ...
  // ---- input: hold-key + press-and-hold on the bubble ----
  // Pointer up/down must be armed from the mic itself. A window-level mouseup used to
  // toggle the panel on every click anywhere in the hub.
  const HOLD_MS = 180;
  let armed = false;
  let held = false;
  let holdTimer = 0;
  ...
  function cancelHold(silent) {
    ...
    if (held || recording) stopRec(silent !== false);
    held = false;
  }

  function beginKeyHold() {
    if (keyArmed || recording || busy || document.hidden) return;
    keyArmed = true;
    held = false;
    clearTimeout(keyTimer);
    keyTimer = setTimeout(() => {
