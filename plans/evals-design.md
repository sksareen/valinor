...
- Writing: drafts that sound like the writer on a good day, with real theses and cited facts — not generic "good writing."

## 2. What exists today (mapped 2026-09-12)

- No eval infra: no golden sets, judges, or scoring. Only unrelated SRS grades in `ingrain.html` and verifier language in `loopkeeper/`.
...
- **Execution: partial trajectories exist.**
  - `<data-home>/execute/tasks.json`: a live backlog with `sourceIngestId` links → real captures in `<data-home>/ingest/`.
  - `<data-home>/execute/events.jsonl`: real chain `scan.started → suggestion.created → promoted/archived → exit.now/agent/schedule`.
