...
const HubClient = (() => {
  ...
  // Walk parent then grandparent so nested embeds (e.g. board.html inside plan.html
...
if (HubClient.embedded) {
  ...
  function applyAppearance(data) {
    ...
    applyHubPad(data);
    if (typeof window.__hubZoom === 'function') window.__hubZoom();
  }
  ...
  addEventListener('storage', (e) => {
    ...
    if (e.key === 'hub-size' && typeof window.__hubZoom === 'function') window.__hubZoom();
  });
  addEventListener('keydown', (e) => {
...
// ---- global text-size bridge: settings S/M/L/XL scales every view ----
// Views use hardcoded px type, so dataset.size alone changes nothing. Zoom the
// document instead — same scale family as the hub chrome. Sources, in order:
// own dataset (appearance posts + parent mutation observer) → parent dataset →
// localStorage (standalone opens) → md (no zoom, today's native sizing).
(function hubZoomBridge() {
  const MAP = { sm: 0.92, md: 1, lg: 1.18, xl: 1.38 };
  function size() {
    const r = document.documentElement.dataset.size;
    if (MAP[r]) return r;
    try {
      const p = window.parent;
      if (p && p !== window) {
        const s = p.document.documentElement.dataset.size;
        if (MAP[s]) return s;
      }
    } catch (e) {}
    try {
      const s = localStorage.getItem('hub-size');
      if (MAP[s]) return s;
    } catch (e) {}
    return 'md';
  }
  function apply() {
    const z = MAP[size()];
    const go = () => { try { document.body.style.zoom = z === 1 ? '' : String(z); } catch (e) {} };
    if (document.body) go();
    else document.addEventListener('DOMContentLoaded', go, { once: true });
  }
  window.__hubZoom = apply;
  apply();
})();
